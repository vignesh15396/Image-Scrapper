...
def search_and_download(search_term: str, driver_path: str, target_path: str, number_images: int):
    target_folder = os.path.join(target_path) # make the folder name inside images with the search string

    if not os.path.exists(target_folder):
        os.makedirs(target_folder) # make directory using the target path if it doesn't exist already

#    with webdriver.Chrome(executable_path=driver_path) as wd:
    with webdriver.Chrome(executable_path='/app/.chromedriver/bin/chromedriver') as wd:
